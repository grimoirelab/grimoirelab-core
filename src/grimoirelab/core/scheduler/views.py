...
@require_http_methods(["POST"])
@csrf_exempt
def github_owner_repos(request):
    owner = json.loads(request.body)['owner']
    api_token = None

    if 'apiToken' in json.loads(request.body):
        api_token = json.loads(request.body)['apiToken']

    try:
        connection = django_rq.get_connection('owner')
        q = Queue('owner', connection=connection)
        job = q.enqueue(fetch_github_owner_repos, owner, api_token)
...
@require_http_methods(["POST"])
@csrf_exempt
def gitlab_owner_repos(request):
    owner = json.loads(request.body)['owner']
    api_token = None
    ...
    try:
        connection = django_rq.get_connection('owner')
        q = Queue('owner', connection=connection)
        job = q.enqueue(fetch_gitlab_owner_repos, owner, api_token)
    except Exception as err:
        return JsonResponse({'status': 'false', 'message': err})
    return JsonResponse({'jobId': job.id})
...
@require_http_methods(["GET"])
def show_owner_job(request, job_id):
    try:
        connection = django_rq.get_connection('owner')
        job = Job.fetch(job_id, connection=connection)
    except NoSuchJobError:
